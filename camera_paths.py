...
def generate_sphere_path(
    camera: Cameras,
    steps: int = 30,
    radius: Optional[float] = None,
    rots: int = 2,
    phi: float = 0.3,  # (-0.5, 0.5)
) -> Cameras:
    """
    Returns a list of camera in a sprial trajectory.
    ...
    Args:
        ...
        radiuses: The list of radii for the spiral in xyz directions.
        rots: The number of rotations to apply to the camera.
        phi: the rad of azimuth angle

    Returns:
        A spiral camera path.
    """
    assert radius is not None, "Either radius or radiuses must be specified."
    if radius is not None:
        rad = torch.tensor(radius, device=camera.device)
    else:
        raise ValueError("Only one of radius or radiuses must be specified.")

    camera = camera.flatten()

    up = torch.tensor([0., 0, 1.], dtype=torch.float, device=camera.device)  # scene is z up
    target = torch.tensor([0, 0, 0], device=camera.device)  # camera looking in -z direction, and the object are in origin

    new_c2ws = []; centers = []
    for theta in torch.linspace(0.0, 2.0 * torch.pi * rots, steps + 1)[:-1]:
        z_rad = rad * torch.cos(torch.tensor(torch.pi * phi))
        center = (
            torch.tensor([torch.cos(theta) * z_rad, 
                          torch.sin(theta) * z_rad, 
                          torch.sin(torch.tensor(torch.pi * phi)) * rad], 
        ...
        centers.append(torch.cat((center, camera_utils.normalize(lookat))))

    # ######################################
    # # Debug
    # cen = torch.stack(centers)
    # np.savetxt('test.xyz', cen.cpu().numpy())
